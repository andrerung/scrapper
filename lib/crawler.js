...
export class WebsiteCrawler {
  ...
  async crawl(startUrl) {
    ...
    try {
      // Continuous crawling: process queue until both queue is empty AND all tasks are done
      // This ensures we process newly discovered links from pages
      console.log(chalk.cyan('🔄 Starting continuous crawl (will follow all discovered links)...\n'));

      let lastQueueCheck = Date.now();
      const queueCheckInterval = 2000; // Check every 2 seconds

      while (this.stats.pagesDownloaded < this.maxPages) {
        // Add items from queue to processing queue
        ...
        // If queue is empty, wait a bit to see if processing discovers new links
        if (this.queue.length === 0) {
          // Check if we have any active tasks
          if (processingQueue.size === 0 && processingQueue.pending === 0) {
            // Both queue empty and no active tasks - we're done!
            console.log(chalk.green('\n✓ Queue exhausted and all pages processed\n'));
            break;
          }

          // Still have active tasks, wait for them to potentially add more links
          const now = Date.now();
          if (now - lastQueueCheck >= queueCheckInterval) {
            console.log(chalk.gray(`  Queue: ${this.queue.length} pending | Processing: ${processingQueue.pending} active | Pages: ${this.stats.pagesDownloaded}/${this.maxPages}`));
            lastQueueCheck = now;
          }

          await delay(1000); // Wait 1 second before checking queue again
        }
      }
  ...
  async processPage(context, url) {
    try {
      ...
      try {
        ...
        // Add new links to queue and track how many were new
        const queueSizeBefore = this.queue.length;
        links.forEach(link => this.addToQueue(link));
        const newLinksAdded = this.queue.length - queueSizeBefore;
        ...
        const newAssetsFound = this.assets.size - assetsBefore;
        ...
        // Log with discovery stats
        const discoveryInfo = [];
        if (newLinksAdded > 0) {
          discoveryInfo.push(chalk.cyan(`+${newLinksAdded} pages`));
        }
        if (newAssetsFound > 0) {
          discoveryInfo.push(chalk.yellow(`+${newAssetsFound} assets`));
        }
        const discoveryStr = discoveryInfo.length > 0 ? ` [${discoveryInfo.join(', ')}]` : '';

        console.log(chalk.green(`✓ [${this.stats.pagesDownloaded}/${this.maxPages}] ${url}${discoveryStr}`));
